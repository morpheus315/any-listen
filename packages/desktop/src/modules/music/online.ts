...
export const getMusicUrl = async ({
  ...
  if (cached && !isRefresh) {
    console.log(`[musicUrl] cache hit: song="${musicInfo.name}" cacheKey=${id} cachedQuality="${cached.quality}"`)
    return { isFromCache: true, quality: cached.quality, url: cached.url }
  }
  console.log(`[musicUrl] cache miss: song="${musicInfo.name}" cacheKey=${id} targetQuality="${targetQuality}"`)
  ...
  console.log(`[musicUrl] fetched: song="${musicInfo.name}" actualQuality="${info.quality}"`)
  return {
    quality: info.quality,
    url: info.url,
    isFromCache: false,
